    ...
    <div className="fixed inset-0 z-[1200] bg-gradient-to-br from-surface to-[#EDEDED] flex items-center justify-center p-6">
      <div className="bg-white border-4 border-[#FFD700] rounded-[24px] p-8 md:p-7 max-w-[480px] w-full text-center shadow-[0_16px_48px_rgba(0,0,0,0.12)] animate-[pop-in_0.4s_cubic-bezier(0.175,0.885,0.32,1.275)]">
        ...
        <div className="grid grid-cols-5 gap-1.5 my-3">
          {Array.from({ length: 10 }, (_, i) => i + 1).map((tableIndex) => {
            ...
            return (
              ...
              >
                <span className="block text-sm text-orange">Table {tableIndex}</span>
        ...
        {isEngineLoaded && engineState.achievements.filter((a) => a.unlockedAt).length > 0 && (
          <div className="mb-3">
            <p className="font-body text-xs text-text-dim mb-1">Badges Earned</p>
        ...
        <p className="font-body text-xs text-text-dim mb-1">
          Stars Earned: {engineState.stars} ⭐
        </p>

        <p className="font-body text-xs text-text-dim">Each star shows how fast you finished</p>
