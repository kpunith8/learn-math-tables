...
**Every user-facing form that submits data must be protected by Turnstile** to keep bots out. Today that's only the guest "Add Name" modal; when you add a new form, use the reusable component + the `POST /api/name` pattern.

- **Reusable component**: `src/components/turnstile.tsx` — `<Turnstile ref={ref} action="addname" />` with a `TurnstileHandle` exposing `getToken()` / `reset()`. It encapsulates everything below (Script loading, portal-retry render, hide-on-solve, `remove()` cleanup). Render it inside the form/dialog itself (NOT on the landing page). It uses `[data-turnstile-name]` with `data-sitekey={process.env.NEXT_PUBLIC_TURNSTILE_SITEKEY}` and the `action` prop (use a distinct `action` per surface). Before submitting, call `turnstileRef.current?.getToken()`; on a failed save call `reset()`.
- **API script**: the component loads `https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit` via `next/script` `strategy="afterInteractive"` + `onLoad`. The render effect bails until the script is loaded, EXCEPT when `window.turnstile` already exists (remount of an already-loaded script — e.g. dialog reopen — does not re-fire `onLoad`).
- **Render pattern**: `window.turnstile?.render(container, { sitekey, action, callback })`. The dialog portal mounts the container in a follow-up render, so retry with `requestAnimationFrame` (up to ~10 frames) until the container exists. Hide the widget once verified — set `display: none` on the container AND its parent wrapper in the `callback` (auto-passing in managed mode; users should never see it after it solves).
- **Cleanup**: the component's effect cleanup calls `window.turnstile?.remove(widgetId)` when it unmounts — otherwise you get `[Cloudflare Turnstile] Cannot find Widget …` in the console (the portal unmounts the container, orphaning the instance).
- **Server-side**: the submit handler must POST the token (key `cf-turnstile-response`) to an API route (e.g. `POST /api/name`) that does a canonical `siteverify` against `https://challenges.cloudflare.com/turnstile/v0/siteverify` with `TURNSTILE_SECRET` (with `AbortSignal.timeout(10_000)`), requires `success === true`, matches `action`, and checks `hostname` against the `TURNSTILE_HOSTNAMES` allowlist (`localhost`, `127.0.0.1`, `mathsadventures.vercel.app`). Do NOT trust the client-side token; verify it server-side.
- **Env**: runtime needs only `TURNSTILE_SECRET`, `NEXT_PUBLIC_TURNSTILE_SITEKEY`, `TURNSTILE_HOSTNAMES`. (`CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` are setup-only.)
- **New routes** that the proxy (`src/proxy.ts`, Kinde `withAuth` `publicPaths`) would 401-gate must add their path to `publicPaths` (e.g. `/api/name`).
- Types for `window.turnstile` (`render`/`reset`/`remove`/`getResponse`) live in `src/types/turnstile.d.ts`.
